...
def _decode_source() -> Tuple[str, str]:
    ...
    if st.session_state.get("dna"):
        choices.append("Original encoded data")
    if st.session_state.get("noisy_dna"):
        choices.append("Noisy encoded data")
    if st.session_state.get("reconstructed_dna"):
        choices.append("Sequencing rebuilt data")

    if not choices:
        return "No DNA available", ""

    previous = st.session_state.get("reconstruction_source", choices[-1])
    if previous not in choices:
        if "Noisy encoded data" in choices:
            previous = "Noisy encoded data"
        elif "Sequencing rebuilt data" in choices:
            previous = "Sequencing rebuilt data"
        else:
            previous = choices[0]
    ...
    elif label == "Sequencing rebuilt data":
        dna = st.session_state.get("reconstructed_dna", "")
    else:
    ...
    with st.container(border=True):
        ...
        with right:
            ...
            if pixel_representation == "Black-white (1 bit/pixel)":
                ...
                st.caption("Black-white is stored as packed bits: 1 pixel = 1 bit.")
...
def render_panel_2_compression() -> None:
    with st.container(border=True):
        ...
        method_options = list_image_compression_methods()
        previous_method = st.session_state.get("robust_image_compression_method", "No compression")
        if previous_method not in method_options:
            previous_method = "No compression"
        method = st.selectbox(
            "Image compression method",
            method_options,
            index=method_options.index(previous_method),
            key="robust_image_compression_method",
        )

        caption = method_caption(method)
        if caption:
            st.caption(caption)
        ...
        quality_rows = [
            ...
            {"Metric": "Payload data" if storage_meta.get("method") == "No compression" else "Compressed payload", "Value": fmt_bytes(payload_size)},
...
def render_panel_4_experiment() -> None:
    with st.container(border=True):
        ...
        with st.expander("Advanced sequencing simulation", expanded=False):
            st.caption("Optional full read-level simulation. You can skip this and reconstruct directly from Original or Noisy encoded data in Step 5.")
...
def render_panel_5_decoding() -> None:
    with st.container(border=True):
        ...
        note = st.session_state.get("decoded_note", "")
        if note:
            st.caption(note)
